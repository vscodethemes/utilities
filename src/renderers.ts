...
export interface LanguageTokenStyle {
  color?: string;
  fontWeight?: string;
  fontStyle?: string;
...
export const renderThemePreviewSvg = ({
  ...
  if (rounded) {
    svg += `<path d="M0 8C0 3.58172 3.58172 0 8 0H452C456.418 0 460 3.58172 460 8V20H0V8Z" fill="${t.titleBarActiveBackground}" />`;
  } else {
    svg += `<path d="M0 0H460V20H0V0Z" fill="${t.titleBarActiveBackground}" />`;
  }
  if (t.titleBarBorder) {
    svg += `<path d="M0 19H460V20H0V19Z" fill="${t.titleBarBorder}" />`;
  }
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    for (let j = 0; j < lineTokens.length; j++) {
      ...
      const style = renderSvgTokenStyle(token.style, t.editorForeground);
